import Image from "next/image";
import Link from "next/link";
import { Button } from "@/components/ui/button";
import { Card, CardContent } from "@/components/ui/card";

export default function Header() {
	return (
		<header className="sticky top-0 z-50 w-full">
			<div className="mx-auto max-w-7xl px-4 py-2 sm:px-6 lg:px-8">
				<Card className="border-border/40 bg-card/60 shadow-lg backdrop-blur-xl">
					<CardContent className="px-4 py-2.5 sm:px-6">
						<div className="flex items-center justify-between gap-4">
							{/* Logo */}
							<Link
								className="flex-shrink-0 transition-opacity hover:opacity-80"
								href="/"
							>
								<Image
									alt="GDG Logo"
									className="h-7 w-auto sm:h-9"
									height={36}
									priority
									src="/gdg-logo.png"
									width={180}
								/>
							</Link>

							{/* Navigation */}
							<nav className="hidden items-center gap-1 md:flex">
								<Button
									asChild
									className="text-foreground/80 hover:bg-accent hover:text-accent-foreground"
									size="sm"
									variant="ghost"
								>
									<Link href="/">Home</Link>
								</Button>
								<Button
									asChild
									className="text-foreground/80 hover:bg-accent hover:text-accent-foreground"
									size="sm"
									variant="ghost"
								>
									<Link href="/about">About Us</Link>
								</Button>
								<Button
									asChild
									className="text-foreground/80 hover:bg-accent hover:text-accent-foreground"
									size="sm"
									variant="ghost"
								>
									<Link href="/events">Events</Link>
								</Button>
								<Button
									asChild
									className="text-foreground/80 hover:bg-accent hover:text-accent-foreground"
									size="sm"
									variant="ghost"
								>
									<Link href="/gallery">Gallery</Link>
								</Button>
								<Button
									asChild
									className="text-foreground/80 hover:bg-accent hover:text-accent-foreground"
									size="sm"
									variant="ghost"
								>
									<Link href="/team">Team</Link>
								</Button>
								<Button
									asChild
									className="ml-2 bg-primary text-primary-foreground shadow-sm hover:bg-primary/90"
									size="sm"
								>
									{/* <Link
                      href="https://gdg.community.dev/gdg-on-campus-gla-university-mathura-india/"
                      target="_blank"
                      rel="noopener noreferrer"
                    >
                      Join Us
                    </Link> */}
								</Button>
							</nav>

							{/* Mobile Menu Button */}
							{/* <div className="flex items-center gap-2 md:hidden">
                  <Button
                    size="sm"
                    className="bg-primary text-primary-foreground shadow-sm hover:bg-primary/90"
                    asChild
                  >
                    <Link
                      href="https://gdg.community.dev/gdg-on-campus-gla-university-mathura-india/"
                      target="_blank"
                      rel="noopener noreferrer"
                    >
                      Join Us
                    </Link>
                  </Button>
                </div> */}
						</div>

						{/* Mobile Navigation */}
						<nav className="mt-3 flex flex-wrap items-center justify-center gap-1.5 border-border/40 border-t pt-3 md:hidden">
							<Button
								asChild
								className="text-foreground/80 text-xs hover:bg-accent hover:text-accent-foreground"
								size="sm"
								variant="ghost"
							>
								<Link href="/">Home</Link>
							</Button>
							<Button
								asChild
								className="text-foreground/80 text-xs hover:bg-accent hover:text-accent-foreground"
								size="sm"
								variant="ghost"
							>
								<Link href="/about">About</Link>
							</Button>
							<Button
								asChild
								className="text-foreground/80 text-xs hover:bg-accent hover:text-accent-foreground"
								size="sm"
								variant="ghost"
							>
								<Link href="/events">Events</Link>
							</Button>
							<Button
								asChild
								className="text-foreground/80 text-xs hover:bg-accent hover:text-accent-foreground"
								size="sm"
								variant="ghost"
							>
								<Link href="/gallery">Gallery</Link>
							</Button>
							<Button
								asChild
								className="text-foreground/80 text-xs hover:bg-accent hover:text-accent-foreground"
								size="sm"
								variant="ghost"
							>
								<Link href="/team">Team</Link>
							</Button>
						</nav>
					</CardContent>
				</Card>
			</div>
		</header>
	);
}
